...
<template>
  <div>
    ...
    <section
      v-if="showHero"
      class="relative min-h-[520px] flex items-center justify-center overflow-hidden"
    ...
    >
      ...
      <div class="relative z-10 text-center px-6 py-20 max-w-[680px] w-full">
        <h1 class="m-0 mb-5 leading-tight">
          ...
          <span
            class="block text-5xl font-bold text-white tracking-wider"
            style="background: linear-gradient(135deg, #fff 0%, #a8c8ff 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;"
          >共创未来</span>
        </h1>
        <p class="text-base text-white/60 m-0 mb-12 tracking-wider">
          寻找志同道合的伙伴，在这里施展才华、实现价值
        </p>
    ...
    <section
      v-if="!showHero"
      class="mt-14 py-12 text-center text-white"
      style="background: linear-gradient(160deg, #0f1b3d 0%, #1a2f6b 40%, #2a4a9e 70%, #3b6bc7 100%)"
    >
      <h1 class="text-3xl font-bold m-0 mb-2">
        {{ recruitType === 'campus' ? '校园招聘' : '社会招聘' }}
      </h1>
      <p class="text-white/60 text-sm m-0">
        {{ recruitType === 'campus' ? '面向应届毕业生与实习生，开启职业生涯' : '面向有工作经验的候选人，欢迎各路人才加入' }}
      </p>
    </section>
    ...
    >
      ...
      <div class="max-w-[1200px] mx-auto px-6 py-16">
        <h2 class="text-2xl font-bold text-text-primary text-center m-0 mb-3">
          为什么选择我们
        </h2>
        <p class="text-sm text-text-muted text-center m-0 mb-12">
          高效连接企业与人才，让招聘更简单
        </p>
      ...
      <div class="max-w-[1200px] mx-auto px-6 pb-16">
        <div class="grid grid-cols-2 gap-6">
          <div
            class="relative overflow-hidden rounded-xl p-8 cursor-pointer group transition-shadow hover:shadow-lg"
            style="background: linear-gradient(135deg, #0f1b3d, #2a4a9e)"
            @click="$router.push({ name: 'experienced' })"
          >
            <div class="relative z-10">
              <h3 class="text-xl font-bold text-white m-0 mb-2">
                社会招聘
              </h3>
              <p class="text-sm text-white/60 m-0 mb-4">
                面向有经验的候选人，发现更好的职业机会
              </p>
            ...
            <div class="absolute right-4 bottom-4 opacity-10">
              ...
              ><rect
                ...
                height="14"
                rx="2"
              /><path d="M16 3h-8l-2 4h12z" /></svg>
            </div>
          </div>
          <div
            class="relative overflow-hidden rounded-xl p-8 cursor-pointer group transition-shadow hover:shadow-lg"
            style="background: linear-gradient(135deg, #1a3a2a, #16a34a)"
            @click="$router.push({ name: 'campus' })"
          >
            <div class="relative z-10">
              <h3 class="text-xl font-bold text-white m-0 mb-2">
                校园招聘
      ...
      <div class="border-t border-border-light">
        <div class="max-w-[1200px] mx-auto px-6 py-10">
          <div class="grid grid-cols-4 gap-8 text-center">
            <div>
              <div class="text-3xl font-bold text-primary m-0 mb-1">
                {{ total > 0 ? total : '100+' }}
              </div>
              <div class="text-sm text-text-muted m-0">
                在招岗位
              </div>
            </div>
            <div>
    ...
    <section class="max-w-[1200px] mx-auto px-6 py-6 pb-12">
      <div class="flex gap-5 items-start">
        ...
        <aside
          ...
          class="w-[220px] shrink-0 bg-white rounded-xl border border-border-light sticky top-[72px]"
        >
          <!-- 岗位分类 -->
          <div class="p-5 border-b border-border-light">
            <h4 class="text-lg font-bold text-text-primary m-0 mb-3">
              岗位分类
            </h4>
          ...
          <div class="p-5">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              >全部</span>
              <span
                v-for="item in JOB_NATURE_MAP.all"
                :key="item.value"
                class="block px-3 py-[7px] text-sm text-text-secondary bg-transparent rounded-md cursor-pointer transition-all hover:text-primary hover:bg-primary-bg select-none"
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center justify-between bg-white rounded-xl px-5 py-3 mb-3 border border-border-light">
            ...
            <span
              v-if="total > 0"
              class="text-sm text-text-secondary shrink-0"
            >共 {{ total }} 个岗位</span>
          </div>
