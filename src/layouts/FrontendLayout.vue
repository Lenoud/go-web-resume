...
<template>
  <div class="min-h-screen bg-bg-page">
    ...
    <header class="fixed top-0 left-0 w-full h-14 bg-white shadow-sm z-16 flex items-center px-6">
      <!-- Logo -->
      <h1
        class="text-xl font-bold text-primary m-0 cursor-pointer mr-8 shrink-0"
            ...
            <template #overlay>
              <a-menu>
                <template v-if="auth.currentUserRole === ROLE.HR">
                  <a-menu-item @click="goUserCenter('myJobView')">
                    岗位管理
                  </a-menu-item>
                  <a-menu-item @click="goUserCenter('companyPostView')">
                    投递管理
                  </a-menu-item>
                </template>
                <template v-else>
                  <a-menu-item @click="goUserCenter('resumeEditView')">
                    我的简历
                  </a-menu-item>
                  <a-menu-item @click="goUserCenter('myPostView')">
                    应聘记录
                  </a-menu-item>
                  <a-menu-item @click="goUserCenter('myOfferView')">
                    我的Offer
                  </a-menu-item>
                </template>
                <a-menu-divider />
                <a-menu-item @click="goUserCenter('userInfoEditView')">
                  编辑资料
                </a-menu-item>
                <a-menu-item @click="goUserCenter('securityView')">
                  账号安全
                </a-menu-item>
                <a-menu-divider />
                <a-menu-item
